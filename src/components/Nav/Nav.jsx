...
import { IoCloseOutline, IoLocationOutline, IoMenuOutline, IoSearchSharp } from "react-icons/io5";
...
const Nav = () => {
  ...
  const isOverlayOpen = isSearchOpen || isOpen || Boolean(activeModal);
  ...
  return (
    <nav className="w-full font-sans fixed left-0 right-0 top-0 z-50">
      <div
        ref={headerRef}
        className={`bg-white shadow-sm transition-transform duration-300 will-change-transform ${
        ...
        <div className="max-w-[1440px] mx-auto">
          ...
          <div className="bg-[#f5f5f5] py-1.5 px-6 md:px-12 flex justify-end items-center text-[12px] font-medium border-b border-gray-200 hidden md:flex">
            <ul className="flex gap-4">
              <li onClick={() => setActiveModal("store")} className="hover:opacity-70 cursor-pointer">
                {t("nav.findStore")}
              </li>
              <div className="w-[1px] h-3 bg-black/20 self-center"></div>
              <li onClick={() => setActiveModal("help")} className="hover:opacity-70 cursor-pointer">
                {t("nav.help")}
              </li>
              <div className="w-[1px] h-3 bg-black/20 self-center"></div>
              <li onClick={() => setActiveModal("join")} className="hover:opacity-70 cursor-pointer">
                {t("nav.joinUs")}
              </li>
              <div className="w-[1px] h-3 bg-black/20 self-center"></div>
              <li
                onClick={() => setActiveModal("signin")}
                className="hover:opacity-70 cursor-pointer font-bold tracking-tight text-black"
              >
                {t("nav.signIn")}
              </li>
              <div className="w-[1px] h-3 bg-black/20 self-center"></div>
              ...
              <div className="flex-shrink-0 relative group">
                ...
                >
                  ...
                  <img
                    src={Logo}
                    alt="Logo"
                    className="relative w-20 md:w-[100px] lg:w-[150px] h-auto object-contain"
                  />
                </Link>
              </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-10">
                      {filteredProducts.map((product) => (
                        <div
                          key={product.id}
                          onClick={() => {
                            navigate(`/catalog?search=${encodeURIComponent(product.title)}`);
                            setIsSearchOpen(false);
                          }}
      ...
      {activeModal === "signin" && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-2xl p-8 shadow-2xl relative animate-in zoom-in duration-200">
            <button onClick={() => setActiveModal(null)} className="absolute top-4 right-4 text-2xl">
              <IoCloseOutline />
            </button>
            <form onSubmit={(e) => handleAuthSubmit(e, "signin")} className="flex flex-col items-center">
              ...
              <input
                required
                type="email"
                placeholder={t("modal.signIn.email")}
                className="w-full border p-3 rounded-md mb-3 outline-black"
              />
              <input
                required
                type="password"
                placeholder={t("modal.signIn.password")}
                className="w-full border p-3 rounded-md mb-6 outline-black"
              />
              <button
                type="submit"
                className="w-full bg-black text-white py-3 rounded-md font-bold hover:bg-gray-800 transition-colors"
              >
                {t("modal.signIn.login")}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-2xl p-8 shadow-2xl relative animate-in zoom-in duration-200 text-center">
            ...
            <form onSubmit={(e) => handleAuthSubmit(e, "join")} className="space-y-3">
              ...
              <button type="submit" className="w-full bg-black text-white py-3 rounded-md font-bold mt-4 hover:bg-gray-800">
                {t("modal.join.join")}
              </button>
            </form>
          </div>
      ...
      {activeModal === "store" && (
        <div className="fixed inset-0 bg-black/60 z-[110] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-2xl p-8 text-center animate-in fade-in duration-300">
            ...
            <button onClick={handleFindStore} className="w-full bg-black text-white py-3 rounded-full font-bold mt-4">
              {t("modal.store.allow")}
            </button>
            <button onClick={() => setActiveModal(null)} className="text-gray-500 py-2 mt-2">
              {t("modal.store.later")}
            </button>
          </div>
        </div>
            ...
            <ul className="space-y-2">
              {(Array.isArray(t("modal.help.items")) ? t("modal.help.items") : []).map((item) => (
                <li
                  ...
                  className="p-3 bg-gray-50 rounded-lg hover:bg-gray-100 cursor-pointer border"
                >
                  {item}
                </li>
              ))}
      ...
      <div
        className={`fixed top-0 right-0 h-full w-[85%] max-w-sm bg-white z-[120] transition-transform duration-500 transform ${
          isOpen ? "translate-x-0" : "translate-x-full"
        } md:hidden p-8`}
      >
        <button type="button" onClick={() => setIsOpen(false)} className="absolute top-6 right-6 text-3xl" aria-label="Close menu">
          <IoCloseOutline />
        </button>
        ...
        <div className="mt-12 pt-12 border-t space-y-4">
          <button onClick={() => { setActiveModal("signin"); setIsOpen(false); }} className="block text-lg font-medium">
            {t("nav.signIn")}
          </button>
          <button onClick={() => { setActiveModal("join"); setIsOpen(false); }} className="block text-lg font-medium">
            {t("nav.joinUs")}
          </button>
          <button onClick={() => { setActiveModal("store"); setIsOpen(false); }} className="block text-lg font-medium">
            {t("nav.findStore")}
          </button>
          <div className="pt-4">
            <select
